...
function getApiBaseUrl() {
  return getBackendInternalBaseUrl("ai-insights summary proxy");
}
...
export async function GET(request: NextRequest) {
  ...
  for (const key of ["hours", "zone", "mode", "provider"]) {
    const value = request.nextUrl.searchParams.get(key);
    if (value) params.set(key, value);
  }

  try {
    const response = await fetch(`${getApiBaseUrl()}/api/ai-insights/summary?${params.toString()}`, {
      method: "GET",
    ...
    return NextResponse.json(
      {
        detail: error instanceof Error ? error.message : "AI insights summary proxy request failed.",
      },
      { status: 502 },
    );
  }

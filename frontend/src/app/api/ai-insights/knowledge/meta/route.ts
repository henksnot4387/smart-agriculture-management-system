...
function getApiBaseUrl() {
  return getBackendInternalBaseUrl("ai-insights knowledge meta proxy");
}
...
export async function GET() {
  ...
  try {
    const response = await fetch(`${getApiBaseUrl()}/api/ai-insights/knowledge/meta`, {
      method: "GET",
    ...
    return NextResponse.json(
      {
        detail: error instanceof Error ? error.message : "AI insights knowledge meta proxy request failed.",
      },
      { status: 502 },
    );
  }

...
function getApiBaseUrl() {
  return getBackendInternalBaseUrl("copilot knowledge meta legacy proxy");
}
...
export async function GET() {
  ...
  try {
    const response = await fetch(`${getApiBaseUrl()}/api/copilot/knowledge/meta`, {
      method: "GET",
    ...
    return NextResponse.json(
      {
        detail: error instanceof Error ? error.message : "Knowledge meta proxy request failed.",
      },
      { status: 502 },
    );
  }

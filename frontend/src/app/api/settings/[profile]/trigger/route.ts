...
export async function POST(_: NextRequest, context: { params: Promise<{ profile: string }> }) {
  ...
  try {
    const response = await fetch(
      `${getBackendInternalBaseUrl("settings trigger proxy")}/api/settings/${profile}/trigger`,
      {
        method: "POST",
    ...
    return NextResponse.json(
      { detail: error instanceof Error ? error.message : "Settings trigger proxy failed." },
      { status: 502 },
    );
  }
}
